...
def test_t03_result_schema(basic_search):
...
def test_t04_result_count(basic_search):
    count = len(results_of(basic_search))
    assert count >= 1
    print(f"\nT-04 result count for 'rust async patterns': {count}")


# --- 2.2 modes ---

def test_t05_mode_realtime(kn):
    data = kn("search", "test query", "--mode", "realtime").yaml()
    assert data["mode"] == "realtime"
    assert "results" in data


def test_t06_mode_pro(kn):
    data = kn("search", "test query", "--mode", "pro").yaml()
    assert data["mode"] == "pro"
    assert "results" in data


def test_t07_no_mode_flag_uses_server_default(kn):
    ...
    print(f"\nT-07 server default mode: {data['mode']}")


def test_t08_invalid_mode(kn):
...
def test_t17_malformed_date(kn):
...
def test_t18_missing_query_arg(kn):
    res = kn("search")
    assert res.code == 2
...
def test_t20_invalid_api_key(kn):
